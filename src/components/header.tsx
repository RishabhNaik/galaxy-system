...
export function Header() {
  ...
  return (
    <>
      ...
      {/* Mobile Drawer Menu (100% Opaque Solid Light Theme with Dark Navy Text) */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-[100] flex flex-col bg-white p-6 text-navy animate-in fade-in duration-200 shadow-2xl overflow-hidden">
          <div className="flex items-center justify-between border-b border-navy/15 pb-4 mb-6">
            ...
            <button
              ...
              className="h-10 w-10 flex items-center justify-center border border-navy/20 bg-slate-100 text-navy hover:bg-slate-200"
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  href={link.href}
                  onClick={() => setMobileMenuOpen(false)}
                  className={`py-3.5 px-4 text-xs font-bold uppercase tracking-wider border transition-all flex items-center justify-between ${
                    isActive
                      ? "bg-navy text-white border-navy"
          ...
          <div className="mt-auto pt-6 border-t border-navy/15 flex flex-col gap-3">
            ...
            <a
              ...
              className="flex items-center justify-center gap-2 py-3 border border-navy/20 bg-slate-50 text-navy text-xs font-bold uppercase tracking-wider hover:bg-slate-100"
            >
              <Phone className="h-4 w-4 text-sky-dark" />
              Call +91 98450 56726
            </a>
          </div>
        </div>
